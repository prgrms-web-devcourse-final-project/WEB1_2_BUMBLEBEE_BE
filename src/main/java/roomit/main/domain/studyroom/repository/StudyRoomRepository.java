...
public interface StudyRoomRepository extends JpaRepository<StudyRoom,Long> {
    @Query("SELECT sr FROM StudyRoom sr JOIN sr.workPlaceId wp WHERE sr.capacity >= :capacity AND wp.workplaceAddress = :workplaceAddress AND NOT EXISTS (SELECT r FROM Reservation r WHERE r.studyRoom = sr AND (r.startTime < :endTime AND r.endTime > :startTime))")
